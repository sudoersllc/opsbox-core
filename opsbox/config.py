...
import sys
from opsbox.plugins import PluginInfo, Registry, Pipeline
...
class ApplicationSettings(BaseModel):
    ...
    modules: list[str] | str = Field(None, description="List of modules to load in pipeline format.")
...
def find_missing_fields(plugins: list[PluginInfo], conf: dict) -> list[tuple[str, str, FieldInfo]]:
    """Finds the missing fields in the configuration.

    Args:
        models (list[BaseModel]): The list of models to check.
        conf (dict): The configuration dictionary.

    Returns:
        list[tuple[str, str, FieldInfo]]: A list of tuples containing the field name, plugin name, and field info.
    """
    missing_fields = []
    models = Registry().grab_conf_models(plugins)

    for model in models:
        for field_name, field_info in model.model_fields.items():
            if field_name not in conf:
                missing_fields.append((field_name, model.__name__, field_info))
...
class Config2(metaclass=SingletonMeta):
    ...
    def _grab_args(self) -> tuple[dict]:
        """Grab the configuration arguments from the environment, command line, or configuration file.

        Returns:
            tuple[dict]: The configuration arguments."""
        total_config = {}
    ...
    def load_env_pipeline(self):
        """Load the environment pipeline.
        This is used to load the pipeline from the environment.

        Returns:
            Pipeline | None: The pipeline if found, None otherwise.
        """
        modules = self.basic_settings.modules
        pipeline = Pipeline(raw_pipeline=modules) if modules is not None else None
        return pipeline

    def find_missing_fields(self, plugins: list[PluginInfo]) -> list[tuple[str, str, FieldInfo]]:
        """Finds the missing fields in the configuration.
        ...
        for model in models:
            for field_name, field_info in model.model_fields.items():
                if field_name not in self.module_settings:
                    missing_fields.append((field_name, model.__name__, field_info))
        return missing_fields
