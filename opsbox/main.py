from opsbox.config import Config2
...
def main():
    try:
        ...
        start_logging(log_level=config.basic_settings.log_level, log_file=config.basic_settings.log_file, verbose=config.basic_settings.verbose)
        reg = Registry(plugin_dir=config.basic_settings.plugin_dir)
        pipeline = config.load_env_pipeline()

        if pipeline is None:
            available_plugins = reg.available_plugins
            print_opsbox_banner()
            print_welcome_message()
            print_pipeline_building_help()
            print_config_help()
            print_basic_args_help()
            print_available_plugins(available_plugins, excluded=(None if "--see_all" in argv else ["handler", "provider"]), plugin_dir=config.basic_settings.plugin_dir)
            sys.exit(1)
        else:
            needed = reg.load_multiple_plugins(pipeline.all_modules, config.module_settings)
            if needed is not None:
                print_opsbox_banner()
                names = [module.name for module in pipeline.all_modules()]
                print_missing_arguments_error(names, needed)
                sys.exit(1)
        ...
        print_opsbox_banner()
        print_plugin_not_found_error(config.basic_settings.plugin_dir, e)
        print_available_plugins(reg.available_plugins, excluded=(None if "--see_all" in argv else ["handler", "provider"]), plugin_dir=config.basic_settings.plugin_dir)
        return 1
    
    # process pipeline
    pipeline.execute(config.module_settings)
